...
class FlowerClient(fl.client.NumPyClient):
    ...
    def __init__(self, trainset, valset, use_mnist: bool):
        self.nohomo = True
        self.weights_unencrypted = True  # The weights recived from init are un-encruypted
    ...
    def get_parameters(self, config):
        self.weights_unencrypted = False
        return encrypt_export_weights(self.model)

    def set_parameters(self, params):
        if self.weights_unencrypted:
            self.model.set_weights(params)
        else:
            self.model = decrypt_import_weights(self.model, params)
