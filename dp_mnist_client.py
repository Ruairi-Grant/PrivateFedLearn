...
class MnistClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        global PRIVACY_LOSS
        if self.dpsgd:
            privacy_spent = flwr_common.compute_epsilon(
                self.local_epochs,
                len(self.x_train),
                self.batch_size,
...
def main(args) -> None:
    # Load Keras model
    model = flwr_common.create_cnn_model()

    # Load a subset of MNIST to simulate the local data partition
    (x_train, y_train), (x_test, y_test) = flwr_common.load(args.num_clients)[args.partition]
